 ...
 */
public class KeyguardUpdateMonitor extends JzsKeyguardUpdateMonitor {
    ...
    private final ArrayList<WeakReference<JzsKeyguardUpdateMonitorCallback>>
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                if (IccCardConstants.State.NETWORK_LOCKED == simArgs.simState) {
                    if (KeyguardUtils.isMediatekSimMeLockSupport()) {
                        ...
                        mHandler.sendMessage(mHandler.obtainMessage(MSG_SIM_STATE_CHANGE,
                                new SimArgs(IccCardConstants.State.READY, simArgs.simId, 0)));
                    }
                } else {
                ...
                ///M: [ALPS01569826] If there are more than 2 phones, the EXTRA_STATE can not represent global phone states.
                //String state = intent.getStringExtra(TelephonyManager.EXTRA_STATE);
                mHandler.sendEmptyMessage(MSG_PHONE_STATE_CHANGED);
    ...
    /* package */ static class BatteryStatus extends JzsBatteryStatus {
        ...
        public BatteryStatus(int index, int status, int level, int plugged, int health) {
        	super(index, status, level, plugged, health);
            this.index = index;
            this.status = status;
            this.level = level;
    ...
    protected void handleScreenTurnedOn() {
        ...
        for (int i = 0; i < count; i++) {
        	JzsKeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onScreenTurnedOn();
            }
        }
    ...
    protected void handlePhoneStateChanged() {
        ///M: [ALPS01569826] If there are more than 2 phones, the EXTRA_STATE can not represent global phone states.
        mPhoneState = TelephonyManager.getDefault().getCallState();
        if (DEBUG) Log.d(TAG, "handlePhoneStateChanged(" + mPhoneState + ")");
        /*
    ...
    private void handleCarrierInfoUpdate(int simId) {
        if (DEBUG) Log.d(TAG, "handleCarrierInfoUpdate: plmn = " + mTelephonyPlmn[simId]
            + ", spn = " + mTelephonySpn[simId]);
    ...
    private void handleSimStateChange(SimArgs simArgs) {
        ...
        if (state != IccCardConstants.State.UNKNOWN && 
            ...
            for (int i = 0; i < mCallbacks.size(); i++) {
            	JzsKeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
                if (cb != null) {
                    cb.onSimStateChanged(state.toString(), simArgs.simId);
                }
            }
        }
    }
    ...
    public void registerCallback(JzsKeyguardUpdateMonitorCallback callback) {
        ...
        mCallbacks.add(new WeakReference<JzsKeyguardUpdateMonitorCallback>(callback));
    ...
    private void sendUpdates(JzsKeyguardUpdateMonitorCallback callback) {
        ...
        for (int i = PhoneConstants.GEMINI_SIM_1; i <= KeyguardUtils.getMaxSimId(); i++) {
            callback.onSimStateChanged(mSimState[i].toString(), i);
        }
        ...
        mPhoneStateListener = new PhoneStateListener() {
           ...
           public void onServiceStateChanged(ServiceState state) {
                if (state != null) {
                    ...
                    final int regVoiceState = state.getRilVoiceRegState();
                    final int regDataState = state.getRilDataRegState();
                    final boolean isSearching = 
                        (regVoiceState == ServiceState.REGISTRATION_STATE_NOT_REGISTERED_AND_SEARCHING)
                        && (regDataState == ServiceState.REGISTRATION_STATE_NOT_REGISTERED_AND_SEARCHING);

                    if (mNetSearching[simId] && (!isSearching)) {
    ...
    // jz
    public String getSimStateString(int simId){
    	return String.valueOf(getSimState(simId));
    }
}
