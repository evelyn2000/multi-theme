...
public class KeyguardSimPukView extends KeyguardAbsKeyInputView
    ...
    private class StateMachine {
        ...
        public void next(boolean autoverify) {
            int msg = 0;
            if (state == ENTER_PUK) {
                if (checkPuk()) {
                    state = ENTER_PIN;
    ...
    private void updateSim(boolean autoverify) {
        if(!autoverify)
            getSimUnlockProgressDialog().show();

        if (mCheckSimPukThread == null) {
            // jz
            final boolean isAutoVerify = autoverify;
