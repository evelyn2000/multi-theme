...
import com.jzs.common.keyguard.IJzsViewMediatorCallback;
...
public class KeyguardViewMediator {
     ...
     * various things.
     */
    public interface ViewMediatorCallback extends IJzsViewMediatorCallback {
        /**
            ...
            synchronized (KeyguardViewMediator.this) {
                if (TelephonyManager.CALL_STATE_IDLE == phoneState  // call ending
                    ...
                    if (DEBUG) Log.d(TAG, "screen is off and call ended, let's make sure the "
                            + "keyguard is showing");
                    doKeyguardLocked(jzsGetPhoneCoverBundle());
                }
            }
        };
            ...
            switch (simState) {
                case ABSENT:
                    ...
                    synchronized (this) {
                        ...
                        } else if (lastWasLocked && isShowing()) {
	                        resetStateLocked(null);
	                    }
                    }
                    break;
                ...
                case NETWORK_LOCKED:
                    ...
                    if (!isShowing()) {
                        ...
                        jzsGetPhoneCoverBundle(options);
                ...
                case READY:
                    ...
                    break;
                case NOT_READY:
                    /// M: [ALPS01582158] remove reset flow since KeyguardSimPinPukView will dismiss itself.
                    /// No need to do this to avoid blocking ui thread.
					if (isShowing() && lastWasLocked) {
                        resetStateLocked(null);
                    }
                    break;
    ...
    public KeyguardViewMediator(Context context, LockPatternUtils lockPatternUtils) {
        ...
        mVoiceWakeupManager.init(context,this) ;
        ...
        synchronized (this) {
            ...
            if (!enabled && mShowing) {
                ...
                if (PowerOffAlarmManager.isAlarmBoot()) {
                    if (DEBUG) Log.d(TAG, "disable Keyguard when alarm boot, ignoring");
                    return;
                }
...
//            if (mHidden != isHidden) {
  //          	if (DEBUG) KeyguardUtils.xlogD(TAG, "handleSetHidden, mHidden="+mHidden+", newHidden="+isHidden);
    //            mHidden = isHidden;
                if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER && !isHidden){
                    resetStateLocked(null);
                }
...
//            }
    ...
    private void doKeyguardLocked(Bundle options) {
        ...
        if (mKeyguardViewManager.isShowing()) {
            ...
            if (com.jzs.utils.ConfigOption.QS_SUPPORT_COVER 
                  && options != null && options.getBoolean("jzscoverclosed")){
                userActivity();
                resetStateLocked(options);
            }
            return;
    ...
    public void dismiss(boolean authenticated) {
        if (DEBUG) Log.d(TAG, "dismiss, authenticated = " + authenticated);
    ...
    public void handleDismiss(boolean authenticated) {
        if (mShowing && !mHidden) {
            mKeyguardViewManager.dismiss(authenticated, jzsGetPhoneCoverBundle());
        }
    }    
    /// @}
        ...
        public void handleMessage(Message msg) {
           /// if (DBG_MESSAGE) KeyguardUtils.xlogD(TAG, "handleMessage enter msg name=" + getMessageString(msg));
            switch (msg.what) {
                ...
                /// M: Mediatek added message end @}
                // jz
                case JZS_PHONECOVER_RESET:
                    handleJzsCoverKeyguardUpdate((Bundle) msg.obj);
                    break;
            }
    ...
    private void handleKeyguardDone(boolean authenticated, boolean wakeup) {
        ...
        ///   Voice Unlock View dismiss -> AntiTheft View shows -> previous Voice Unlock dismiss flow calls handleKeyguardDone -> remove AntiTheft View
        ///   So we avoid handleKeyguardDone if AntiTheft is the current view and not yet unlock correctly.
        if(AntiTheftManager.isAntiTheftLocked() || mUpdateMonitor.isSimPinSecure()) {
            KeyguardUtils.xlogD(TAG, "handleKeyguardDone() - Skip keyguard done! antitheft = " 
                    + AntiTheftManager.isAntiTheftLocked() + " or sim = " + mUpdateMonitor.isSimPinSecure());
            return ;
        }
        ...
        if (!authenticated && PowerOffAlarmManager.isAlarmBoot()) {
            if (DEBUG) Log.d(TAG, "handleKeyguardDone() skip keyguard done when alarm boot.");
            return;
        }
    ...
    private void handleShow(Bundle options) {
        synchronized (KeyguardViewMediator.this) {
            ...
            if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER
                    && options != null){
                adjustStatusBarLocked(options.getBoolean("jzscoverclosed"));
            } else {
                adjustStatusBarLocked();
            }
            userActivity();
    ...
    void adjustStatusBarLocked(){
        if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER 
             && mJzsPhoneCoverState == android.view.WindowManagerPolicy.WindowManagerFuncs.LID_CLOSED){
    ...
    void adjustStatusBarLocked(boolean iscoverclosed) {
        ...
        if (mStatusBarManager == null) {
            ...
            if (mShowing) {
                ...
                if (isSecure || iscoverclosed || !ENABLE_INSECURE_STATUS_BAR_EXPAND) {
                ...
                if (!isAssistantAvailable() || PowerOffAlarmManager.isAlarmBoot()|| isDisableSearch) {
  ...
  /// M: For DM lock feature to update keyguard
    ...
    /**
     * M: If the keyguard currently showing and going to be hidden, the SIM Pin view won't correctly show.
     * We will need to use this API to guarantee the PIN/PUK/ME lock will be shown anyway (ALPS01472600).
     */
    ...
    public void setShowNextViewAfterSimLock(boolean show) {        
        mKeyguardViewManager.setShowNextViewAfterSimLock(show);
    } 
	private static final int JZS_PHONECOVER_RESET = 100;
    private int mJzsPhoneCoverState = android.view.WindowManagerPolicy.WindowManagerFuncs.LID_ABSENT;
    /* @hide */
    public void setJzsPhoneCoverStatus(int status, boolean forcehandle) {
        if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER){
			android.util.Log.i("Jzs.QsLog", "Mediator::setJzsPhoneCoverStatus()==old:"+mJzsPhoneCoverState+", new:"+status);
            mJzsPhoneCoverState = status;
            if(forcehandle){
                handleJzsPhoneCoverStatus();
            }
        }
    }
    /* @hide */
    public Bundle jzsGetPhoneCoverBundle(){
        if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER){
            Bundle options = new Bundle();
            jzsGetPhoneCoverBundle(options);
            return options;
    ...
    private void jzsGetPhoneCoverBundle(Bundle options){
        if(com.jzs.utils.ConfigOption.QS_SUPPORT_COVER && options != null){
            android.util.Log.v("Jzs.QsLog", "Mediator::jzsGetPhoneCoverBundle()==status:"+mJzsPhoneCoverState);
            if(mJzsPhoneCoverState == android.view.WindowManagerPolicy.WindowManagerFuncs.LID_CLOSED){
                options.putBoolean("jzscoverclosed",  true);
                options.putBoolean("jzsforcerecrete", true);
            } else {
                options.putBoolean("jzscoverclosed",  false);
            }
        }
    }

    /* @hide */
    public void handleJzsPhoneCoverStatus() {
        Message msg = mHandler.obtainMessage(JZS_PHONECOVER_RESET);
        msg.obj = jzsGetPhoneCoverBundle(); 
        mHandler.sendMessage(msg);
    }
    
    protected void handleJzsCoverKeyguardUpdate(Bundle options) {
        //Log.d(TAG, "handleJzsCoverKeyguardUpdate lock=" + closed);
        if (!isShowing() /*&& options != null && options.getBoolean("jzscoverclosed")*/) {
