...
public class KeyguardHostView extends KeyguardViewBase {
    ...
    protected void onDetachedFromWindow() {
        ...
        removeCallbacks(mUpdateKeyguardLayerRunnable);
        if (!KeyguardViewMediator.isKeyguardInActivity) {
    ...
    private KeyguardSecurityCallback mCallback = new KeyguardSecurityCallback() {
        ...
        public void updateKeyguardLayerVisibility(boolean visible) {
            KeyguardHostView.this.updateKeyguardLayerVisibility(visible);
        }
        ...
        public boolean onClickHandler(final View view,
            ...
            if (pendingIntent.isActivity()) {
                hostView.setOnDismissAction(new OnDismissAction() {
                    public boolean onDismiss() {
                        ...
                        try {
                            ...
                            Log.d(TAG, "Keyguard launch app, intent=" + fillInIntent.getAction());
                ...
                if (hostView.mViewStateManager.isChallengeShowing()) {
                    ...
                    Log.d(TAG, "widget onClick to dismiss Keyguard, view=" + view);
                    hostView.dismiss();
                }
                return true;
            } else {
    ...
    private void showSecurityScreen(SecurityMode securityMode) {
        ...
        if (oldView != null) {
            ...
            if (oldView instanceof KeyguardSelectorView && !(newView instanceof KeyguardSelectorView) && mSlidingChallengeLayout != null) {
                updateKeyguardLayerVisibility(false);
        ...
        if (newView instanceof KeyguardSelectorView && mSlidingChallengeLayout != null) {
            updateKeyguardLayerVisibility(true);
    ...
    public void onScreenTurnedOff() {
        ...
        ///   [ALPS01581620] we should set mCurrentPage of PagedView properly before call showChallenge() to shrink the widget.
        mSwitchPageRunnable.run();
        /// M @}
        ...
        if (mSlidingChallengeLayout != null) {
            ...
            /// M: [ALPS01480605] to force to show challenge
            mSlidingChallengeLayout.showChallenge(true);
            /// M: [ALPS01527106] to force set back handle alpha as 1
    ...
    public boolean handleBackKey() {
        ...
        /// M: [ALPS01486919] Remove physical BACK key handler
        /**
    ...
    public void dismiss(boolean authenticated) {
        Log.d(TAG, "dismiss(authenticated = " + authenticated + ") is called.") ;
        /// M: If dm lock is on, should not let user by pass lockscreen
        if (!AntiTheftManager.isDismissable()){
            return;
        }
    ...
    private void updateKeyguardLayerVisibility(boolean visible) {
        mKeyaugerLayerVisibility = visible;
        if (mKeyaugerLayerVisibility) {
            post(mUpdateKeyguardLayerRunnable);
        } else {
            mUpdateKeyguardLayerRunnable.run();
        }
    }

    private Runnable mUpdateKeyguardLayerRunnable = new Runnable() {
        public void run() {
            KeyguardUtils.xlogD(TAG, "mUpdateKeyguardLayerRunnable start");
            if (mSlidingChallengeLayout != null) {
