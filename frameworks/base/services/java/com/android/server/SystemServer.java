...
/// @}
/// M: add for Passpoint R2 Service @{
...
class ServerThread {
    ...
    public void initAndLoop() {
        ...
        ///M: add for Passpoint R2 feature
        IPasspointManager passpointManager = null;
        ...
        /// M: RecoveryManagerService  @{
        if(FeatureOption.MTK_ANTIBRICKING_LEVEL != 0) {
            try {
                Slog.i(TAG, "Recovery Manager");
                recovery = MediatekClassFactory.createInstance(IRecoveryManagerService.class, context);
                if (recovery != null) {
                    ServiceManager.addService(Context.RECOVERY_SERVICE, recovery.asBinder());
                    recovery.startBootMonitor();
                }            
            } catch (Throwable e) {
                ...
                if (recovery != null) {
                    try {
                        if (ex.getStackTrace().length > 0) {
                            ...
                            recovery.parseException(exceptionClassName, throwMethodName, throwClassName);
                        }
                    } catch (Exception exc) {
                        reportWtf("Failure parseException", exc);
                    }
                } else {
		...
		com.jzs.common.JzsServiceInterface jzsPluginMgr = null;
        ...
        if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
            ...
            if ("1".equals(SystemProperties.get("ro.mtk_sensorhub_support"))) {
                try {
                    ISensorHubService sensorHubService = null;
                    try {
                        Slog.i(TAG, "SensorHub Service");
                        sensorHubService = MediatekClassFactory.createInstance(ISensorHubService.class, context);
                    } catch (Exception e) {
                        Slog.e(TAG, "SystemServer Exception=" + e.getClass().getName(), e);
                    }
                    Slog.d(TAG, "SensorHubService=" + sensorHubService);
                    if(sensorHubService != null) {
                        ServiceManager.addService(ISensorHubManager.SENSORHUB_SERVICE, sensorHubService.asBinder());
                    }
                } catch (Throwable e) {
			...
			if(com.jzs.utils.ConfigOption.ENABLE_JZS_JARS){
                try {
                    try {
                        ...
                        Object jzsMgr = com.jzs.common.JzsClassFactory.createServiceInstance(
                            com.jzs.common.JzsClassFactory.SERVER_MANAGER, 
                            context);
                        //Object jzsMgr = com.jzs.common.JzsClassFactory.createServiceInstance(
                        //    com.jzs.common.plugin.IPluginManager.PLUGIN_MANAGER_SERVICE, 
                        //    context);
                        
                        if(jzsMgr != null) {
                            //ServiceManager.addService(com.jzs.common.plugin.IPluginManager.PLUGIN_MANAGER_SERVICE, (IBinder)jzsMgr);
                            jzsPluginMgr = (com.jzs.common.JzsServiceInterface)jzsMgr;
                        }
                        
                    } catch (Exception e) {
                        Slog.e(TAG, "jzs systemServer Exception=" + e.getClass().getName());
                    }
                    //Slog.d("Jzs.Server", "plugin manager=" + jzsPluginMgr);
                    
                } catch (Throwable e) {
                    Slog.e(TAG, "jzs Failure starting plugin manager service.", e);
                }
            ...
            if (FeatureOption.MTK_PASSPOINT_R2_SUPPORT) {
                try {
                    Slog.i(TAG, "Passpoint Service");
                    passpointManager = MediatekClassFactory.createInstance(
                            IPasspointManager.class, context);
                    ServiceManager.addService(Context.MTK_PASSPOINT_SERVICE,
                            passpointManager.asBinder());
                } catch (Throwable e) {
                    Slog.e(TAG, "Failure starting passpointManager", e);
                }
            }
        }
        ...
        final IRecoveryManagerService recoveryF = recovery;
		final com.jzs.common.JzsServiceInterface jzsPluginMgrF = jzsPluginMgr;
        ...
        ActivityManagerService.self().systemReady(new Runnable() {
            public void run() {
				...
				if(com.jzs.utils.ConfigOption.ENABLE_JZS_JARS){
		            try {
		                //com.jzs.common.JzsServiceInterface jzsPluginMgrF
		                if (jzsPluginMgrF != null) {
		                    jzsPluginMgrF.systemReady();
		                }
		            } catch (Throwable e) {
		                reportWtf("making jzs PluginManagerService ready", e);
		            }
		        }
