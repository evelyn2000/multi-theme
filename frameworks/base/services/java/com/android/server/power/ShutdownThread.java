...
public final class ShutdownThread extends Thread {
    ...
    static void shutdownInner(final Context context, boolean confirm) {
        ...
        final int resourceId = mRebootSafeMode
            ...
            : (mReboot ? com.android.internal.R.string.qs_shutdown_reboot_confirm : 
                    (longPressBehavior == 2
            ...
            sConfirmDialog = new AlertDialog.Builder(context)
                .setTitle(mRebootSafeMode
                        ? com.android.internal.R.string.reboot_safemode_title
                            : (mReboot ? com.android.internal.R.string.factorytest_reboot : com.android.internal.R.string.power_off))
     ...
     */
    public static void sbPowerOffDialogRedraw(final Context context) {
        if (sConfirmDialog != null) {
            sConfirmDialog.dismiss();
    ...
    private static void beginShutdownSequence(Context context) {
        ...
        if(!mShutOffAnimation){
   			try{
				File f_shut = new File("/data/local/shutanimation.zip");
				if (f_shut.exists()) {
					mShutOffAnimation = true;
				} else {
					String animfile = SystemProperties.get("persist.qs.shutanimation","shutanimation.zip");
                    if(!animfile.equals("shutanimation.zip")){
                        f_shut = new File("/system/media/shutant", animfile);
                    } else {
					    f_shut = new File("/system/media", animfile);
                    }
					if (f_shut.exists()) {
						mShutOffAnimation = true;
					}
				...
				Log.w(TAG, "shutanimation.zip isnt exists");
			}
    ...
    private static void checkShutdownFlow() {
        ...
        final String IPODisableProp = SystemProperties.get("sys.ipo.disable");
        boolean isIPOEnabled= !IPODisableProp.equals("1");
        final String encryptionProgress = SystemProperties.get("vold.encrypt_progress");
        boolean isEncrytionInProgress = (!encryptionProgress.equals("100") && !encryptionProgress.equals(""));

        Log.d(TAG, "checkShutdownFlow: IPO_Support=" + FeatureOption.MTK_IPO_SUPPORT +
                " mReboot=" + mReboot +
                " sys.ipo.disable=" + IPODisableProp +
                " encryptionProgress= " + encryptionProgress);
        if (FeatureOption.MTK_IPO_SUPPORT == false || mReboot == true || Zygote.systemInSafeMode == true || isIPOEnabled== false || isEncrytionInProgress == true) {
    ...
    public void run() {
        ...
        while (mShutdownFlow == IPO_SHUTDOWN_FLOW) {
            ...
            // start launcher to improve shutdown performance and make the original top activity enter pause.
    ...
    public void running() {
        ...
        /// @} 2012-05-20
        mContext.sendOrderedBroadcastAsUser((new Intent()).setAction(Intent.ACTION_SHUTDOWN)
                .putExtra("_mode", mShutdownFlow).addFlags(Intent.FLAG_RECEIVER_FOREGROUND),
    ...
    public static void rebootOrShutdown(boolean reboot, String reason) {
    	final String encryptionProgress = SystemProperties.get("vold.encrypt_progress");
      boolean isEncrytionInProgress = false;
        ...
        //add for MFR
		if(com.mediatek.common.featureoption.FeatureOption.MTK_HDMI_SUPPORT){
	        try {
	            if (ImHDMI == null)
        ...
        sInstance.mPowerManager.setBacklightBrightnessOff(true);
        isEncrytionInProgress = (!encryptionProgress.equals("100") && !encryptionProgress.equals(""));
        if(isEncrytionInProgress == true){
        	PowerManagerService.lowLevelShutdown();
        }else{
        SystemProperties.set("ctl.start", "shutdown");
        }	
