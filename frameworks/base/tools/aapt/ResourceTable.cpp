        ...
        for (size_t i=0; i<N; i++) {
            ...
            if (id != 0) {
                ...
                if (id >= mNextPackageId) {
					if(phase == 0 && (id > mNextPackageId) /*&& !name.compare(String16("com.jzs"))*/){
					//	printf("====jz===addIncludedResources(2): \"%s\" skip plus id=%d, IsApp=%d, HaveApp=%d, IsmtkCur=%d, CurNum=%d, nextid=%d, \n",
               		//			String8(name).string(), id, mIsAppPackage, mHaveAppPackage, mIsmtkCur, mIsmtkCurNum, mNextPackageId);
						
						continue;
					} else {
					//printf("====jz===addIncludedResources(3): \"%s\" skip plus id=%d, IsApp=%d, HaveApp=%d, IsmtkCur=%d, CurNum=%d, nextid=%d, \n",
               		//			String8(name).string(), id, mIsAppPackage, mHaveAppPackage, mIsmtkCur, mIsmtkCurNum, mNextPackageId);
...
sp<ResourceTable::Package> ResourceTable::getPackage(const String16& package)
{
    ...
    if (p == NULL) {
        if (mIsAppPackage) {
			...
			if (mIsmtkCur){
				// jz changed for other platform
				#if 0 //ndef __MTK_PLATFORM__
				if(mIsmtkCurNum > mNextPackageId){
