...
#ifndef JZ_CUSTOM_STYLE
#define JZ_CUSTOM_STYLE
#endif
...
bool AaptGroupEntry::getJzStyleName(const char* name, ResTable_config* out)
{
#ifdef JZ_CUSTOM_STYLE
	/*if (strcmp(name, kWildcardName) == 0) {
        if (out) out->jzStyle = 0;
        return true;
    }*/
    const char* c = name;
//	printf("getJzStyleName(0): %s\n", c);
	
    if (tolower(*c) != 'q') return false;
    c++;
    if (tolower(*c) != 's') return false;
    c++;
    if (tolower(*c) != 's') return false;
    c++;
...
//printf("getJzStyleName(1): %s->%s\n", name, c);
    ...
    int d = atoi(val);
//	printf("getJzStyleName(2): %s=%d\n", val, d);
    if (d > 0) {
        if (out) out->jzStyle = d;
        return true;
    }
#endif
    return false;
...
bool AaptGroupEntry::getJzCustomerName(const char* name, ResTable_config* out)
{
#ifdef JZ_CUSTOM_STYLE
	/*if (strcmp(name, kWildcardName) == 0) {
        if (out) out->jzCustomer = 0;
        return true;
    }*/
	
    const char* c = name;
//	printf("getJzCustomerName(0): %s\n", c);
    if (tolower(*c) != 'q') return false;
    c++;
    if (tolower(*c) != 's') return false;
    c++;
...
//printf("getJzCustomerName(1): %s\n", c);
    ...
    int d = atoi(val);
//	printf("getJzCustomerName(3): %s=%d\n", val, d);
    if (d > 0) {
        if (out) out->jzCustomer = d;
        return true;
    }
#endif
    return false;
...
AaptGroupEntry::parseNamePart(const String8& part, int* axis, uint32_t* value)
{
	...
	// jz
	#ifdef JZ_CUSTOM_STYLE
	if(getJzStyleName(part.string(), &config)){
		*axis = AXIS_QS_STYLE;
        *value = config.jzStyle;
		return 0;
	}
	
	if(getJzCustomerName(part.string(), &config)){
		*axis = AXIS_QS_CUSTOMER;
        *value = config.jzCustomer;
		return 0;
	}
	#endif
	// jz ....
...
AaptGroupEntry::getConfigValueForAxis(const ResTable_config& config, int axis)
{
    switch (axis) {
		...
		#ifdef JZ_CUSTOM_STYLE
		case AXIS_QS_STYLE:
			return config.jzStyle;
		case AXIS_QS_CUSTOMER:
			return config.jzCustomer;
		#endif
    }
    return 0;
}
...
AaptGroupEntry::initFromDirName(const char* dir, String8* resType)
{
    ...
    *resType = part;
	...
	if (getJzStyleName(part.string())) {
        ...
        //printf("initFromDirName(JzStyle) false: %s\n", part.string());
    }

	if (getJzCustomerName(part.string())) {
        jzcustomer = part;
        ...
        //printf("initFromDirName(JzCustomer) false: %s\n", part.string());
    }
	#endif
	// jz	
...
int AaptGroupEntry::compare(const AaptGroupEntry& o) const
{
	...
	// jz
	#ifdef JZ_CUSTOM_STYLE
	if (v == 0) v = jzStyle.compare(o.jzStyle);
    if (v == 0) v = jzCustomer.compare(o.jzCustomer);
	#endif
	//
...
const ResTable_config& AaptGroupEntry::toParams() const
{
	...
	// jz
	#ifdef JZ_CUSTOM_STYLE
	getJzStyleName(jzStyle.string(), &params);
	getJzCustomerName(jzCustomer.string(), &params);
	#endif
