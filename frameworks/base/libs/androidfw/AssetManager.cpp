...
#if !defined(THEME_NO_BUILD)
#include <cutils/properties.h>
#endif /* !defined(THEME_NO_BUILD) */
#endif
...
// jz changed
bool AssetManager::addAssetPath(const String8& path, void** cookie, const char* pkg)
{
    AutoMutex _l(mLock);
	...
	if(checAndGetAssetFilePath(ap.path, pkg))
		return true;
	#endif
...
// jz changed
void AssetManager::setConfiguration(ResTable_config& config, const char* locale)
{
    AutoMutex _l(mLock);
    *mConfig = config;
    if (locale) {
...
#else /* !defined(THEME_NO_BUILD) */
    strncpy(kThemeApkPath, kDefaultThemePath, 100);
#endif /* !defined(THEME_NO_BUILD) */
...
// jz changed
#ifdef JZS_SUPPORT_LY_STYLE
void AssetManager::jzsAddAssetFilePath(const char* path, const char* idmap){
	asset_path oap;
	oap.path.setTo(path);
	oap.type = kFileTypeRegular;
	oap.idmap.setTo(idmap);
    mAssetPaths.add(oap);
}

bool AssetManager::checAndGetAssetFilePath(const String8& path, const char* pkg){
	return false;
}
#endif