...
void ResTable_config::copyFromDtoH(const ResTable_config& o) {
	...
	// jz
    #if 1
    jzStyle = dtohs(jzStyle);
    jzCustomer = dtohs(jzCustomer);
    #endif
}
...
void ResTable_config::swapHtoD() {
	...
	// jz
    #if 1
    jzStyle = htods(jzStyle);
    jzCustomer = htods(jzCustomer);
    #endif
}
...
int ResTable_config::compare(const ResTable_config& o) const {
    ...
    #if 1
    if (diff != 0) return diff;
    diff = (int32_t)(jzCustomConfig - o.jzCustomConfig);
    #endif
...
int ResTable_config::compareLogical(const ResTable_config& o) const {
    // jz
    #if 1
    if (jzStyle != o.jzStyle) {
        return (jzStyle < o.jzStyle) ? -1 : 1;
    }

    if (jzCustomer != o.jzCustomer) {
        return (jzCustomer < o.jzCustomer) ? -1 : 1;
    }
    #endif
    // jz end
    if (mcc != o.mcc) {
...
int ResTable_config::diff(const ResTable_config& o) const {
	...
	//jz
    #if 1
    if (jzCustomConfig != o.jzCustomConfig) diffs |= CONFIG_UI_MODE;
    #endif
...
bool ResTable_config::isMoreSpecificThan(const ResTable_config& o) const {
    ...
    // jz
    #if 1
    if (jzCustomConfig || o.jzCustomConfig) {
        if (jzStyle != o.jzStyle) {
            if (!jzStyle) return false;
            if (!o.jzStyle) return true;
        }

        if (jzCustomer != o.jzCustomer) {
            if (!jzCustomer) return false;
            if (!o.jzCustomer) return true;
        }
    }
    #endif
    // jz end
...
bool ResTable_config::isBetterThan(const ResTable_config& o,
    ...
    if (requested) {
        ...
        #if 1
        if (jzCustomConfig || o.jzCustomConfig) {
            if ((jzStyle != o.jzStyle) && requested->jzStyle) {
                return (jzStyle);
            }

            if ((jzCustomer != o.jzCustomer) && requested->jzCustomer) {
                return (jzCustomer);
            }
        }
        #endif
...
bool ResTable_config::match(const ResTable_config& settings) const {
    ...
    #if 1
    if (jzCustomConfig != 0) {
        if (jzStyle != 0 && jzStyle != settings.jzStyle) {
            return false;
        }
        if (jzCustomer != 0 && jzCustomer != settings.jzCustomer) {
            return false;
        }
    }
    #endif
...
///M: Add for DFO resolution feature {@
//   Description is deatailed in declaration in *.h file
#if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)
bool ResTable_config::matchforDFOResolution(const ResTable_config& settings, uint16_t dfoDpi) const {
        
    if( (density != DENSITY_DEFAULT) && (density != DENSITY_NONE) ){
        switch(dfoDpi){
            case ResTable_config::DENSITY_LOW:
                if (density != ResTable_config::DENSITY_LOW){
                    TABLE_GETENTRY(ALOGI("For ldpi device, filtering out res other than ldpi density(%d) when DFO resolution feature is opened", density));
                    return false;
                }
                break;
            case ResTable_config::DENSITY_MEDIUM:
                if (density != ResTable_config::DENSITY_MEDIUM){
                    TABLE_GETENTRY(ALOGI("For mdpi device, filtering out res other than mdpi density(%d) when DFO resolution feature is opened", density));
                    return false;
                }
                break;
            case ResTable_config::DENSITY_HIGH:
                if (density != ResTable_config::DENSITY_HIGH){
                    TABLE_GETENTRY(ALOGI("For hdpi device, filtering out res other than hdpi density(%d) when DFO resolution feature is opened", density));
                    return false;
                }
                break;
            case ResTable_config::DENSITY_XHIGH:
                if ((density != ResTable_config::DENSITY_HIGH) && (density != ResTable_config::DENSITY_XHIGH)){
                    TABLE_GETENTRY(ALOGI("For xhdpi device, filtering out res other than hdpi/xhdpi density(%d) when DFO resolution feature is opened", density));
                    return false;
                }
                break;
            case ResTable_config::DENSITY_XXHIGH:
                if ((density != ResTable_config::DENSITY_HIGH) && (density != ResTable_config::DENSITY_XHIGH) && (density != ResTable_config::DENSITY_XXHIGH)){
                    TABLE_GETENTRY(ALOGI("For xxhdpi device, filtering out res other than hdpi/xhdpi/xxhdpi density(%d) when DFO resolution feature is opened", density));
                    return false;
                }
                break;
            default:                    
                TABLE_GETENTRY(ALOGI("DFO resolution unknown, treat as non-DFO case"));
                return true;
        }
    }
    return true;
}
#endif //(MTK_DFO_RESOLUTION_SUPPORT) && !(MTK_FOR_HOST_BUILD_AAPT)
///@}
...
String8 ResTable_config::toString() const {
	...
	if (jzStyle != 0) {
        ...
        res.appendFormat("qss%d", dtohs(jzStyle));
    }
    if (jzCustomer != 0) {
        if (res.size() > 0) res.append("-");
        res.appendFormat("qsc%d", dtohs(jzCustomer));
    }
	#endif
    ...
    const Type** outTypeClass) const
{
    ...
    for (size_t i=0; i<NT; i++) {
        ...
        ///M:{@
        #if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)
        //Check to make sure this one is valid for the current parameters when DFO resolution feature is opened
        
        bool isNativeBuildApk = false;
        bool isFilterableType = true;
        String8 pkgName(pkg->name);
        String8 resTypename;
        size_t  typeLen_temp;

        const char16_t* restype = package->typeStrings.stringAt(typeIndex, &typeLen_temp);
        resTypename.setTo(restype,typeLen_temp);
        
        if (getInfoForDfoReslution(pkgName,resTypename, isNativeBuildApk, isFilterableType)!= NO_ERROR){
            ALOGW("Error getting Info of DFO Resolution!\n");
            return BAD_VALUE;
        }

        if(isNativeBuildApk && isFilterableType){
            if (config && !thisConfig.matchforDFOResolution(*config,mCurrentDFODpi)) {
                TABLE_GETENTRY(ALOGI("Does not match config for DFO resolution, so skip it!\n"));
                continue;
            }
        }
        #endif //(MTK_DFO_RESOLUTION_SUPPORT) && (!MTK_FOR_HOST_BUILD_AAPT)
...
///M :add for DFO_RESOLUTION feature {@
#if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)

Vector<String8> ResTable::mMtkPkgList;
uint16_t        ResTable::mCurrentDFODpi = 0;

///M add: for set up current DFO dpi, return DENSITY_NONE when there's something wrong
uint16_t ResTable::getDFODpi() const
{
    char propVal[PROPERTY_VALUE_MAX];
    String8 screenResolution;
    if (property_get("persist.sys.current.screensize", propVal, "nothing") > 0){
        screenResolution.setTo(propVal);
        if(screenResolution.compare(String8("nothing"))==0){
            // Get default resolution if "[persist.sys.current.screensize" is not set yet(DFO is never set before this point)
            int dfo_w = 800, dfo_h = 480;
            int pixels = dfo_w * dfo_w;
                        
            #if defined(LCM_WIDTH) && defined(LCM_HEIGHT)
                //get directly from ProjectConfig.mk
                dfo_w = atoi(LCM_WIDTH);
                dfo_h = atoi(LCM_HEIGHT);
                pixels = dfo_w * dfo_h;
                //ALOGW("[DFO]getDFODpi from define w,h is [%d,%d], pixel is %d\n", dfo_w,dfo_h,pixels);
            #else
                ALOGE("[DFO]LCM_WIDTH and LCM_HEIGHT not defined, fail to get default resolution");
                return  ResTable_config::DENSITY_NONE;
            #endif
            
            if (pixels <= 240 * 432){                
                return ResTable_config::DENSITY_LOW; //<= WQVGA432
            } else if ((pixels >= 480 * 800) && (pixels <=1024 * 600)){                
                return ResTable_config::DENSITY_HIGH; // >= WVGA854
            } else if ((pixels > 1024 * 600) && (pixels <1920 * 1080)){                
                return ResTable_config::DENSITY_XHIGH; // >= 720p
            } else if (pixels >= 1920 * 1080){                
                return ResTable_config::DENSITY_XXHIGH; // >= FHD
            } else {                
                return ResTable_config::DENSITY_MEDIUM; //  default
            }
        } else if(screenResolution.compare(String8("480x320"))==0){
            //HVGA => mdpi
            return ResTable_config::DENSITY_MEDIUM;
        } else if (screenResolution.compare(String8("800x480"))==0){
            //WVGA => hdpi
            return ResTable_config::DENSITY_HIGH;
        } else if (screenResolution.compare(String8("854x480"))==0){
            //FWVGA => hdpi
            return ResTable_config::DENSITY_HIGH;
        } else if (screenResolution.compare(String8("960x540"))==0){
            //qHD => hdpi
            return ResTable_config::DENSITY_HIGH;
        } else if (screenResolution.compare(String8("1280x720"))==0){
            //720p => xhdpi
            return ResTable_config::DENSITY_XHIGH;
        } else if (screenResolution.compare(String8("1920x1080"))==0){
            //fhd => xxhdpi
            return  ResTable_config::DENSITY_XXHIGH;
        } else {
            //unknown resolution
            ALOGW("Unknown resolution: %s, from persist.sys.current.screensize)\n", String8(propVal).string());
            return  ResTable_config::DENSITY_NONE;
        }
    } else {
        ALOGW("[DFO] read persist fail");
        return  ResTable_config::DENSITY_NONE;
    }
}

///M: add for judge if DFO resolution should apply on this apk
status_t ResTable::getInfoForDfoReslution(String8 &pkgName, String8 &resTypeName,                                           
                                          bool &isNativeBuildApk, bool &isFilterableType) const
{
    //Check if is Filterable res type
    if(resTypeName.compare(String8("mipmap"))==0){
        //mipmap type should not be filtered
        isFilterableType = false;
        return NO_ERROR;
    }    
    //Get current dfoDpi
    mCurrentDFODpi = getDFODpi();

    if (mCurrentDFODpi == ResTable_config::DENSITY_NONE){
        ALOGE("[DFO] DFO resolution gets an unknown dpi\n");
        return BAD_VALUE;
    }
    //ALOGW("[DFO] mCurrentDFODpi from getDFODpi() is %d",mCurrentDFODpi);

    //Check if it isNativeBuildApk
    if( (pkgName.compare(String8("android"))==0) || (pkgName.compare(String8("com.mediatek"))==0) ){
        //FW package is native build package
        isNativeBuildApk = true;
        return NO_ERROR;
    }
    if (mMtkPkgList.isEmpty()){
	    //  We only allow the mMtkPkgList is empty when
	    //  1. Zygote preload, this case should be filtered already by package name
	    //  2. it is booting up, which PMS is not ready yet (filtered by uid = 1000	    
	    if (getuid() == 1000){
	        //ALOGW("[DFO] At this time PMS is not ready yet");
	        return NO_ERROR;
	    } else {
	        //Should never get here
	        ALOGW("[DFO] mMtkPkgList is empty,something is wrong ");
	        return BAD_VALUE;
	    }
    } else {
        // UidArray has been set up, then do the comparing
        for (uint8_t i=0 ; i < mMtkPkgList.size(); i++){
            if( pkgName.compare(mMtkPkgList[i])==0 ){
		        isNativeBuildApk = true;		        
		        return NO_ERROR;
		    }
        }
    }
    //ALOGW("[DFO] Doesnot match Native pkg name, %s is a non Native pkg,uid is %d", pkgName.string(),getuid());
    return NO_ERROR;
}
#endif//(MTK_DFO_RESOLUTION_SUPPORT) && !(MTK_FOR_HOST_BUILD_AAPT)
///@}
