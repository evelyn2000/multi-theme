...
import com.jzs.common.IPreferenceHelper;
...
public class Preference implements Comparable<Preference> {
    ...
    public Preference(Context context, AttributeSet attrs, int defStyle) {
        ...
        if(com.jzs.utils.ConfigOption.QS_THEME_SUPPORT){
			initializeJzsHelper(context, attrs);
            mLayoutResId = getJzsDefaultLayoutResId(mLayoutResId);
        }
    ...
    protected View onCreateView(ViewGroup parent) {
		...
		onJzsCreateView(parent, layout, widgetFrame);
    ...
    protected void onBindView(View view) {
		...
		onJzsBindView(view, titleView, summaryView, imageView);
    ...
    public void setOrder(int order) {
        ...
        if (order != mOrder) {
            ...
            if(changeJzsOrder(order, mOrder))
            	return;
    ...
    public int compareTo(Preference another) {
        if (mOrder != another.mOrder) {
			// jz
			int ret = compareJzsTo(mOrder, another.mOrder);
			if(ret != 0){
	            return ret;
			}
            // Do order comparison
	...
	protected void initializeJzsHelper(Context context, AttributeSet attrs){
		if(context.getContextHelper() != null){
    		mPreferenceHelper = context.getContextHelper().createPreferenceHelper(this, attrs);
        }
    }
	/*  {@hide}     */
	public IPreferenceHelper getPreferenceHelper(){
		return (IPreferenceHelper)getJzsHelper();
	}
	/*  {@hide}     */
	...
	protected int getJzsDefaultLayoutResId(int dr){
		if(getPreferenceHelper() != null)
			return getPreferenceHelper().getDefaultLayoutResId(dr);
		
		return dr;
	}
	
	protected void onJzsCreateView(ViewGroup parent, View layout, ViewGroup widgetFrame){
		if(getPreferenceHelper() != null)
			getPreferenceHelper().onCreateView(parent, layout.getViewHelper(), widgetFrame);
	}
	
	protected void onJzsBindView(View view, View title, View summary, View icon){
		if(getPreferenceHelper() != null){
			getPreferenceHelper().onBindView(view.getViewHelper(), title, summary, icon);
        }
	}
	/*  {@hide}     */
	public boolean changeJzsOrder(int order, int curOrder){
		if(getPreferenceHelper() != null){
            return getPreferenceHelper().changeOrder(order, curOrder);
        }
		return false;
	}
	/*  {@hide}     */
	public int compareJzsTo(int order, int another){
		if(getPreferenceHelper() != null){
            return getPreferenceHelper().compareTo(order, another);
        }
		
		return 0;
	}
	/*  {@hide}     */
    public Preference getJzsPreference(){
    	return this;
    }
}
