...
public final class AssetManager {
    ...
    //M: check if DFO pkglist has been set
    private static boolean sDFOResolutionSet = false;
    ...
    public final int addAssetPath(String path, String pkgName) {
        int res = addAssetPathNative(path, pkgName);
        
        //M: setup mtk built package at this point
        if(FeatureOption.MTK_DFO_RESOLUTION_SUPPORT){
            if((res != 0) && (sDFOResolutionSet == false)){
                if(setMtkBuiltPackageList() == true){
                    sDFOResolutionSet = true;                    
                }
            }
        }
        return res;
    }
    private native final int addAssetPathNative(String path, String pkgName);
    
    /**
     * M: Setup Mtk build Package name list for DFO resolutoin      
     */
    private final boolean setMtkBuiltPackageList() {        
        IPackageManager mPm = IPackageManager.Stub.asInterface(ServiceManager.getService("package"));
        if(mPm == null){
            //Log.d(TAG, "[DFO] PkgManager does not exist.");
            return false;
        }
        
        InputStream inStream = null;
        List<String> pkgNameArrayList = new ArrayList<String>();
        try {
            ...
            if (inStream != null) {
                ...
                while (line != null) {
                    String[] packageNames = mPm.getPackagesForUid(Integer.valueOf(line));
                    for(int i=0; i<packageNames.length;i++){
                        if (packageNames[i].equals("com.android.inputmethod.latin") == false)
                            pkgNameArrayList.add(packageNames[i]);
                    }
                   line = buffReader.readLine();
                }
                String[] pkgNameArray = new String[ pkgNameArrayList.size() ];
                pkgNameArrayList.toArray(pkgNameArray);
                setMtkBuiltPkgListNative(pkgNameArray);
            }            
            return true;
            
        } catch (IOException e) {
            Log.w(TAG, "[DFO] resmon-uid.txt IOException!!");            
            return false;
        } catch (RemoteException e){
            Log.w(TAG, "[DFO] getPackagesForUid RemoteException!!", e);            
            return false;
        } finally {
            if (inStream != null) {
                try {
     ...
     /**
     * M: Setup Mtk build Package name list for DFO resolutoin-native
     */
    private native final void setMtkBuiltPkgListNative(String[] packageNameList);
