...
#ifdef JZS_SUPPORT_LY_STYLE
extern jint jzs_jni_android_content_AssetManager_addAssetPathWidthPkg(AssetManager* am, 
							JNIEnv* env, const char* path, jstring jpkg);
extern AssetManager* jzs_jni_android_content_AssetManager_init(JNIEnv* env);
#endif 
                                                       ...
                                                       jstring path, jstring jpkg)
{
	...
	#ifdef JZS_SUPPORT_LY_STYLE
	if(jpkg)
		return jzs_jni_android_content_AssetManager_addAssetPathWidthPkg(am, env, path8.c_str(), jpkg);
	#endif
    void* cookie;
...
static void android_content_AssetManager_setMtkBuiltPkgListNative(JNIEnv* env, jobject clazz, jobjectArray pkgNameStringArray)
{
    AutoMutex _l(gUIDMutex);
    if(ResTable::mMtkPkgList.isEmpty()){
        //obtain each packageName and saved to ResTable
        int stringCount = env->GetArrayLength(pkgNameStringArray);

        for (int i=0; i<stringCount; i++) {
            jstring javaString = (jstring) env->GetObjectArrayElement(pkgNameStringArray, i);
            const char *nativeString = env->GetStringUTFChars(javaString, 0);
            if(nativeString == NULL){
                jniThrowException(env, "java/lang/NullPointerException", "[DFO] get null ptr from GetStringUTFChars");
                return;
            } else {
                //ALOGV("[DFO]getString %s (Java object %p)\n", String8(nativeString).string(), clazz);
                String8 nStinrg(nativeString);
                ResTable::mMtkPkgList.push(nStinrg);
                //Now release them
...
{
#ifdef JZS_SUPPORT_LY_STYLE
	AssetManager* am = jzs_jni_android_content_AssetManager_init(env);
#else
...
static JNINativeMethod gAssetManagerMethods[] = {
    ...
    { "addAssetPathNative", "(Ljava/lang/String;Ljava/lang/String;)I",
        (void*) android_content_AssetManager_addAssetPath },
    { "isUpToDate",     "()Z",
        (void*) android_content_AssetManager_isUpToDate },
            
    #if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)
    { "setMtkBuiltPkgListNative",  "([Ljava/lang/String;)V",
        (void*) android_content_AssetManager_setMtkBuiltPkgListNative },
    #endif //(MTK_DFO_RESOLUTION_SUPPORT) && !(MTK_FOR_HOST_BUILD_AAPT)
