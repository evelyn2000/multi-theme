...
extern "C" int register_com_jzs_lingyin_jni_interface(JNIEnv *env);
#endif
...
//#if defined(WITH_JIT) && defined(WITH_JAZZ)
#if defined(WITH_JAZZ)
extern void dvmCompilerShutdown();
#endif
...
static void removeHprofEmptyFile(void)
{
    char propBuf[PROPERTY_VALUE_MAX];

    property_get("dalvik.vm.oome-hprof-path", propBuf, "");
    if (propBuf[0] == '\0')
        return;
    ...
    while ((de = readdir(dir)))  {
        ...
        if (de->d_type == DT_REG)  {
            if (!strncmp(de->d_name + strlen(de->d_name) - 6, ".hprof", 6))  {
                snprintf(buf, 256, "%s/%s", propBuf, de->d_name);
                ALOGD("unlink %s", buf);
                if (unlink(buf) != 0)  {
                    ALOGD("unlink %s : %d (%s)", buf, errno, strerror(errno));
                }
            }
        }
    }
...
static unsigned long getPhysicalMemorySize(void)
{
    const unsigned long SIZE_256MB = (256L * 1024 * 1024);

    unsigned long memSize = (sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGESIZE));
    memSize = (memSize & ~SIZE_256MB) ? (((memSize >> 28) + 1) << 28) : memSize;

    return memSize;
}

static void setVmHeapSize(void)
{
    ...
    unsigned long memSize = getPhysicalMemorySize() >> 20;
...
//#if defined(WITH_JIT) && defined(WITH_JAZZ)
#if defined(WITH_JAZZ)
    ALOGD("Shutting down compiler thread before exiting ...\n");
    dvmCompilerShutdown();
#endif    
...
#ifdef JZS_SUPPORT_LY_STYLE
	register_com_jzs_lingyin_jni_interface(env);
#endif	
    return 0;
}
