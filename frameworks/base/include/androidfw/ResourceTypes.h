...
struct ResTable_config
{
    ...
    //   When DFO resolution feature is opened
    //   to ensure that the "DFO load" behavior is the same as in "non-DFO load"(release load),
    //   seeking for resource will not fallback to the res with density that will not be built in this project    
    bool matchforDFOResolution(const ResTable_config& settings, uint16_t dfoDpi) const;
    #endif //(MTK_DFO_RESOLUTION_SUPPORT) && !(MTK_FOR_HOST_BUILD_AAPT)
...
#if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)
    //pkgname list for mtk built in apps
    static Vector<String8>     mMtkPkgList;    
    //density setting that set from DFO
    static uint16_t            mCurrentDFODpi;
#endif// MTK_DFO_RESOLUTION_SUPPORT && !(MTK_FOR_HOST_BUILD_AAPT)
...
private:
    ...
    #if defined (MTK_DFO_RESOLUTION_SUPPORT) && !defined(MTK_FOR_HOST_BUILD_AAPT)
    status_t getInfoForDfoReslution(String8 &pkgName,String8 &resTypename, bool &isNativeBuildApk, bool &isFilterableType) const;
    uint16_t getDFODpi() const;
    #endif// MTK_DFO_RESOLUTION_SUPPORT && !(MTK_FOR_HOST_BUILD_AAPT)
