...
$(shell build/tools/findleaves.py --prune=$(OUT_DIR) --prune=.repo --prune=.svn --prune=.git \
...
$(hide) $(AAPT) package --auto-add-overlay $(PRIVATE_AAPT_FLAGS) -m \
...
$(hide) jar $(if $(strip $(PRIVATE_JAR_MANIFEST)),-cfm,-cf) \
    $@ $(PRIVATE_JAR_MANIFEST) $(if $(USE_DXCACHE),`find $(PRIVATE_CLASS_INTERMEDIATES_DIR) -type f -printf "%P\n" | sort | sed -e "s?^? -C $(PRIVATE_CLASS_INTERMEDIATES_DIR) ?"`,-C $(PRIVATE_CLASS_INTERMEDIATES_DIR) .)
endef
...
@mkdir -p $(dir $@)
$(hide) $(if $(USE_DXCACHE),$(dxcache)) $(DX) \
...
$(hide) $(AAPT) package --auto-add-overlay -u $(PRIVATE_AAPT_FLAGS) \
...
$(hide) python mediatek/build/tools/proguard_shrink_parser.py $(PRIVATE_ANDROID_MANIFEST) $(PRIVATE_CLASS_INTERMEDIATES_DIR)/../shrink.txt $(PRIVATE_CLASS_INTERMEDIATES_DIR)/../shrink_important.txt
endef
