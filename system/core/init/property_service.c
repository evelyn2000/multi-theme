...
struct {
    ...
    { "ril.",             AID_RADIO,    0 },
    { "persist.ril.gprs.",AID_RADIO,    0 },
    { "persist.ril.gprs.",AID_SYSTEM,   0 },
    { "mux.",             AID_RADIO,    0 },
    { "mux.",             AID_SYSTEM,   0 },
...
#ifdef MTK_BOX_AUDIO_SUPPORT
    { "box.audio.", AID_MEDIA, 0 },
#endif
    { "volte.emergency.",AID_RADIO, 0 },
    { "persist.service.atci.",AID_RADIO, 0 },
    { NULL, 0, 0 }
};

static const char *qs_allow_property_perms[] = {
    "persist.qs.",
    NULL
};
...
#if defined(MTK_USER_ROOT_SWITCH) || defined(MTK_ATCI_EM_SUPPORT)
    { "atcid-daemon-u", AID_SHELL, AID_SHELL},
    { "atcid-daemon-u", AID_RADIO, AID_RADIO},
    { "atci_service", AID_SHELL, AID_SHELL},
    { "atci_service", AID_RADIO, AID_RADIO},
#endif    
#if defined(EVDO_DT_VIA_SUPPORT)
    { "viaril-daemon", AID_RADIO, AID_RADIO},
    { "ctclientd", AID_RADIO, AID_RADIO},
    { "pppd_via", AID_RADIO, AID_RADIO},
#endif
#if MTK_IMS_SUPPORT 
    { "volte_stack", AID_RADIO, AID_RADIO },
    { "volte_ua", AID_RADIO, AID_RADIO },
    { "volte_imcb", AID_RADIO, AID_RADIO },
#endif
    {NULL, 0, 0 }
};
...
static int check_perms(const char *name, unsigned int uid, unsigned int gid, char *sctx)
{
    ...
    if(!strcmp(name,"ro.secure") || !strcmp(name,"ro.adb.secure") || !strcmp(name,"persist.service.atci.usermode") || !strcmp(name, "persist.user2root.root")
	...
	for (i = 0; qs_allow_property_perms[i]; i++) 
    {
        if (strncmp(qs_allow_property_perms[i], name, strlen(qs_allow_property_perms[i])) == 0) 
        {
            return 1;
        }
    }
...
int property_set(const char *name, const char *value)
{
    ...
    if(pi != 0) {
#ifdef MTK_USER_ROOT_SWITCH
       if(!strncmp(name, "ro.", 3) && strcmp(name,"ro.secure") && strcmp(name,"ro.adb.secure") && strcmp(name,"persist.service.atci.usermode") && strcmp(name,"ro.debuggable")) {
#ifdef MTK_INIT
        ...
        if (ret < 0) {
#ifdef INIT_ENG_BUILD
            ERROR("Failed to set '%s'='%s' pa->bytes_used\n", name, value, pa->bytes_used);
#else
...
// add for WWOP project
#ifdef MTK_CIP_SUPPORT
	load_properties_from_file("/custom/cip-build.prop");
#endif
