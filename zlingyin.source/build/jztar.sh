...
#				cp -f ${MKTOPDIR}/out/target/product/${TARGET_PRODUCT}/EBR2 ${DEST_IMG_DIR}/
	...
	if [ -e ${MKTOPDIR}/zlingyin.source/build/tools/cksmgen/CKSM_Gen ]; then
		chmod a+x ${MKTOPDIR}/zlingyin.source/build/tools/cksmgen/CKSM_Gen
  	${MKTOPDIR}/zlingyin.source/build/tools/cksmgen/CKSM_Gen ${DEST_IMG_DIR} ${DEST_IMG_DIR}
  fi
