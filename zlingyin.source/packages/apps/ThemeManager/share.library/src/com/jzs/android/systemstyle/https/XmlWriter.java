...
public class XmlWriter {
    ...
    public XmlWriter(String path) {
        ...
        try {
            ...
            e.printStackTrace();
            Util.Log.e(TAG, "can't create FileOutputStream");
        }
     ...
     */
    public void close() {
        Util.Log.d(TAG, "[close]XML writer close");
        try {
            if (mSerializer != null) {
            ...
            if (mFileos != null) {
                mFileos.getFD().sync();
                Util.Log.i(TAG, "[close] mFileos.getFD().sync()");
                mFileos.close();
            }
        } catch (IOException e) {
            e.printStackTrace();
            Util.Log.e(TAG, "[close]error occurred while close xml file");
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
        } catch (IllegalStateException e) {
            e.printStackTrace();
    ...
    public void writeStartTag(String tag) throws IOException {
        if (tag == null || mSerializer == null) {
        	Util.Log.e(TAG, "could not write null start tag");
            return;
        }
    ...
    public void writeEndTag(String tag) throws IOException {
        if (tag == null || mSerializer == null) {
        	Util.Log.e(TAG, "could not write null end tag");
            return;
        }
    ...
    public void addValue(String tag, String value) throws IOException {
        if ((tag == null) || (value == null) || mSerializer == null) {
        	Util.Log.e(TAG, "[addValue] could not write null tag or value");
            return;
        }
        Util.Log.w(TAG, "[addValue] TAG: " + tag + ", value: " + value);
        try {
            mSerializer.startTag(null, tag);
