...
public class PackageInfoReader {
    ...
    public PackageInfoReader(Context context, String file) {
        ...
        if (configFileInSystem.exists()) {
        	Util.Log.d(TAG, "[PackageInfoReader]configure.xml exists in system");
            try {
                mConfigureParser = new XmlParser(file);
            } catch (SAXException e) {
            	Util.Log.e(TAG, "[PackageInfoReader] SAXException");
            } catch (IOException e) {
            	Util.Log.e(TAG, "[PackageInfoReader] IOException");
            }
        }
    ...
    public String getOem() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser oem from " + Util.InfoXmlTags.XML_TAG_OEM);
            oem = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_OEM);
            Util.Log.d(TAG, "oem = " + oem);
        }
        return oem;
    }
    ...
    public String getProduct() {
        String product = null;
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser Product from " + Util.InfoXmlTags.XML_TAG_PRODUCT);
            product = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_PRODUCT);
            Util.Log.d(TAG, "Product = " + product);
        }
        return product;
    }
    ...
    public String getFlavor() {
        String flavor = null;
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser flavor from " + Util.InfoXmlTags.XML_TAG_FLAVOR);
            flavor = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_FLAVOR);
            Util.Log.d(TAG, "flavor = " + flavor);
        }
        return flavor;
    }
    ...
    public String getLanguage() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser language from " + Util.InfoXmlTags.XML_TAG_LANGUAGE);
            language = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_LANGUAGE);
            Util.Log.d(TAG, "language = " + language);
        }
        return language;
    }
    ...
    public String getOperator() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser operator from " + Util.InfoXmlTags.XML_TAG_OPERATOR);
            operator = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_OPERATOR);
            Util.Log.d(TAG, "operator = " + operator);
        }
        return operator;
    }
    ...
    public String getPublishTime() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser Publish Time from " + Util.InfoXmlTags.XML_TAG_PUBLISH_TIME);
            publishTime = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_PUBLISH_TIME);
            Util.Log.d(TAG, "Publish Time = " + publishTime);
        }
        return publishTime;
    }
    ...
    public String getAndroidNum() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser getAndroidNum from " + Util.InfoXmlTags.XML_TAG_ANDROID_NUM);
            androidNum = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_ANDROID_NUM);
            Util.Log.d(TAG, "getAndroidNum = " + androidNum);
        }
        return androidNum;
    }
    ...
    public String getVersionName() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser getVersionName from " + Util.InfoXmlTags.XML_TAG_VERSION_NAME);
            versionName = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_VERSION_NAME);
            Util.Log.d(TAG, "getVersionName = " + versionName);
        }
        return versionName;
    }
    ...
    public String getNotes() {
        String notes = null;
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser Notes from " + Util.InfoXmlTags.XML_TAG_NOTES);
            notes = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_NOTES);
            Util.Log.d(TAG, "Notes = " + notes);
        }
        return notes;
    }
    ...
    public String getFingerprint() {
        ...
        if (mConfigureParser != null) {
        	Util.Log.d(TAG, "Parser fingerprint from " + Util.InfoXmlTags.XML_TAG_FINGERPRINT);
            fingerprint = mConfigureParser.getValByTagName(Util.InfoXmlTags.XML_TAG_FINGERPRINT);
            Util.Log.d(TAG, "fingerprint = " + fingerprint);
        }
        return fingerprint;
    }

    public UpdatePackageInfo getInfo(int id) {
    	Util.Log.d(TAG, "[getInfo] begin " + id);
        ...
        if (id >= nodeList.size() || id < 0) {
        	Util.Log.e(TAG, "[getInfo], order >= nodeList.size() || id < 0, size is = " + nodeList.size());
            return null;
        }
    ...
    public List<UpdatePackageInfo> getInfoList() {
    	Util.Log.d(TAG, "[getUpdateInfoList] begin");
        ...
        if (mConfigureParser != null) {
            ...
            Util.Log.d(TAG, "[getInfoList]node list's length: " + nodeList.size());
    ...
    private UpdatePackageInfo getInfoFromNode(Node node) {
        if (node == null) {
        	Util.Log.w(TAG, "[getInfoFromNode]node is null");
            return null;
        }

        Util.Log.v(TAG, "[getInfoFromNode]node name: " + node.getNodeName());
        NodeList childNodeList = node.getChildNodes();/* new ArrayList<Node>(); */
        int length = childNodeList.getLength();
        Util.Log.v(TAG, "[getInfoFromNode]childNodeList list's length: " + length);

        if (length != UpdatePackageInfo.FIELD_NUMBER) {
        	Util.Log.w(TAG, "[getInfoFromNode], child node number is not currect");
            return null;
        }
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (Util.InfoXmlTags.XML_TAG_PUBLISH_TIME.equals(nodeName)) {
                ...
                Util.Log.v(TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]publish time:"
                        + info.publishTime);
                ...
                Util.Log.v(TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]mAndroidNum:"
                        + info.androidNumber);
            } else if (Util.InfoXmlTags.XML_TAG_VERSION_NAME.equals(nodeName)) {
                info.version = child.getTextContent();
                Util.Log.v(TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]version:"
                        + info.version);
            } else if (Util.InfoXmlTags.XML_TAG_NOTES.equals(nodeName)) {
                info.notes = child.getTextContent();
                Util.Log.v(TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList] have notes");
            } else if (Util.InfoXmlTags.XML_TAG_PATH.equals(nodeName)) {
                info.path = child.getTextContent();
                Util.Log.v(TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]mPath:" + info.path);
            } else {
            	Util.Log.w(TAG, "[getInfoFromNode], child node TAG is not currect");
                return null;
            }
        }
