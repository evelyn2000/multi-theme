 ...
 */
//@Widget
public class BounceCoverFlow extends Gallery {
...
//        a.recycle();
    ...
    private int getCenterOfCoverflow() {
        return ((getMeasuredWidth() - getPaddingLeft() - getPaddingRight()) >> 1) + getPaddingLeft();
    }
    ...
    protected int getCenterOfChildWithScroll(View child) {
        return ((child.getLeft() + child.getWidth() / 2) - getScrollX());
    }
    ...
    protected boolean getChildStaticTransformation(View child, Transformation t) {
        ...
        if (childCenter == getCenterOfCoverflow()) {
            ...
            if (DBG) {
                Log.d(TAG, "getChildStaticTransformation: distToCenter = " + distToCenter
                        + ",childCenter = " + childCenter + ",center cover flow = "
                        + getCenterOfCoverflow() + ",rotationAngle = " + rotationAngle
                        + ",mScrollX = " + getScrollX() + ",child = " + child);
            }
    ...
    private void transformImageBitmap(final View child, final Transformation t, final int rotationAngle) {
        if (DBG) {
            Log.d(TAG, "transformImageBitmap: rotation " + rotationAngle);
        }
        ...
        if (DBG) {
            Log.d(TAG, "transformImageBitmap: zoomAmount " + zoomAmount + ",rotationAngle = "
